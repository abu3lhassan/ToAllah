...
async function router(){
  ...
  if(hash.startsWith('#/zakat')) return renderTemplate('zakatTemplate', setupZakat);
...
function zakatCalculation(){ const khams = document.getElementById('khamsSelect').value; const people = Number(document.getElementById('peopleCount').value); const category = document.getElementById('zakatCategory').value; const amount = document.getElementById('amount'), vat = document.getElementById('vat'), total = document.getElementById('total'); const vatBox = document.getElementById('vatBox'), totalBox = document.getElementById('totalBox'); const showKhams = khams === '2'; vatBox.classList.toggle('hidden', !showKhams); totalBox.classList.toggle('hidden', !showKhams); if(!people || !category){ amount.textContent = vat.textContent = total.textContent = '-'; return; } const [base, khamsAmount] = zakahPrices(category); const baseTotal = base * people; const khamsTotal = khamsAmount * people; amount.textContent = baseTotal + ' ﷼'; vat.textContent = khamsTotal + ' ﷼'; total.textContent = (baseTotal + khamsTotal) + ' ﷼'; }
function zakahPrices(index){ switch(index){ case '1': return [30,8]; case '2': return [24,6]; case '3': return [21,5]; case '4': return [48,12]; case '5': return [39,10]; case '7': return [15,4]; case '8': return [9,3]; default: return [0,0]; } }
function zakatMessage(){ const khamsText = document.getElementById('khamsSelect').selectedOptions[0]?.textContent || '-'; const people = document.getElementById('peopleCount').value || '-'; const cat = document.getElementById('zakatCategory').selectedOptions[0]?.textContent || '-'; const amount = document.getElementById('amount').textContent; const vat = document.getElementById('vat').textContent; const total = document.getElementById('total').textContent; const showKhams = document.getElementById('khamsSelect').value === '2'; return `حاسبة زكاة الفطرة\nمخمس: ${khamsText}\nعدد الأفراد: ${people}\nالصنف: ${cat}\nالمبلغ: ${amount}${showKhams ? `\nالخمس: ${vat}\nالمجموع: ${total}` : ''}`; }
